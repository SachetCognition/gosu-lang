...
import gw.gosudoc.com.sun.tools.doclets.internal.toolkit.AbstractDoclet;
...
public class HtmlDoclet extends AbstractDoclet
{
     ...
     * @see gw.gosudoc.com.sun.javadoc.RootDoc
     */
    protected void generateOtherFiles( gw.gosudoc.com.sun.javadoc.RootDoc root, ClassTree classtree)
            throws Exception {
     ...
     * {@inheritDoc}
     */
    protected void generateClassFiles( gw.gosudoc.com.sun.javadoc.ClassDoc[] arr, ClassTree classtree) {
        Arrays.sort(arr);
        for(int i = 0; i < arr.length; i++) {
            if (!(configuration.isGeneratedDoc(arr[i]) && arr[i].isIncluded())) {
                continue;
            }
            gw.gosudoc.com.sun.javadoc.ClassDoc prev = (i == 0)?
                null:
                arr[i-1];
            gw.gosudoc.com.sun.javadoc.ClassDoc curr = arr[i];
            gw.gosudoc.com.sun.javadoc.ClassDoc next = (i+1 == arr.length)?
                null:
                arr[i+1];
            try {
    ...
    gw.gosudoc.com.sun.javadoc.PackageDoc getNamedPackage( List<gw.gosudoc.com.sun.javadoc.PackageDoc> list, int idx) {
        if (idx < list.size()) {
            gw.gosudoc.com.sun.javadoc.PackageDoc pkg = list.get(idx);
            if (!pkg.name().isEmpty()) {
                return pkg;
            }
        }
    ...
    protected void generatePackageFiles(ClassTree classtree) throws Exception {
        Set<gw.gosudoc.com.sun.javadoc.PackageDoc> packages = configuration.packages;
        if (packages.size() > 1) {
            PackageIndexFrameWriter.generate(configuration);
        }
        List<gw.gosudoc.com.sun.javadoc.PackageDoc> pList = new ArrayList<>(configuration.packages);
        gw.gosudoc.com.sun.javadoc.PackageDoc prev = null, next;
        for (int i = 0; i < pList.size(); i++) {
            // if -nodeprecated option is set and the package is marked as
            // deprecated, do not generate the package-summary.html, package-frame.html
            // and package-tree.html pages for that package.
            gw.gosudoc.com.sun.javadoc.PackageDoc pkg = pList.get(i);
    ...
    private void performCopy(String filename) {
        ...
        try {
            ...
            configuration.message.notice((gw.gosudoc.com.sun.javadoc.SourcePosition) null,
                    "doclet.Copying_File_0_To_File_1",
                    fromfile.toString(), path.getPath());
            toFile.copyFile(fromfile);
        } catch (IOException exc) {
            configuration.message.error((gw.gosudoc.com.sun.javadoc.SourcePosition) null,
